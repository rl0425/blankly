...
export default async function ProjectStatsPage({
  ...
  const difficultyAccuracy = {
    easy:
      difficultyStats.easy.length > 0
        ? Math.round(
            difficultyStats.easy.reduce((sum, r) => sum + r.accuracyRate, 0) /
              difficultyStats.easy.length
          )
        : 0,
    medium:
      difficultyStats.medium.length > 0
        ? Math.round(
            difficultyStats.medium.reduce((sum, r) => sum + r.accuracyRate, 0) /
              difficultyStats.medium.length
          )
        : 0,
    hard:
      difficultyStats.hard.length > 0
        ? Math.round(
            difficultyStats.hard.reduce((sum, r) => sum + r.accuracyRate, 0) /
              difficultyStats.hard.length
          )
        : 0,
  };
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {difficultyAccuracy.easy}% ({difficultyStats.easy.length}개
                    방)
                  </span>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    ...
                    {difficultyStats.medium.length}개 방)
                  </span>
                </div>
                <div className="h-3 bg-muted rounded-full overflow-hidden">
                  <div
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {difficultyAccuracy.hard}% ({difficultyStats.hard.length}개
                    방)
                  </span>
                </div>
                <div className="h-3 bg-muted rounded-full overflow-hidden">
                      ...
                      {room.session?.is_completed ? (
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {room.session.correct_count}/
                            {room.session.total_problems}
                          </p>
                        </div>
                      ) : (
